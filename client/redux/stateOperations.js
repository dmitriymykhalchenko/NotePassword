...
export function addItem(state, value) { //????
  let newState = state
  const item = fromJS(value)// in immutable
  console.log(item)
...
export function updateItem(state, value) {
  let newState = state
  const index = state.get('index')//получаем порядковий номер індекс
...
export function resetaddItem(state, value) {
  const newState = state.set('note', List())//присваюем пустой лист
...
export function deleteaddItem(state) {
  ...
  const index = newState.get('index') // получаем индекс итем
  console.log(index)
  const oldnote = newState.get('note') //старий масив ноте

  console.log(oldnote.toJS())//став простий массив
  console.log(oldnote)
  const newNote = oldnote.splice(index, 1) //удалили 1 елемент по індексу з массива и получили новый массыв ноте
...
export function updateaddItem(state, index) {
  ...
  newState = newState.updateIn(['note'], (oldNote) => {
    const newNote = oldNote.splice(index, 1)// обновляем массив и удаляем позицию індекса
    return newNote
  })
  return newState // замена старого стате
}
